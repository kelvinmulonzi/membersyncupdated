...
def auto_checkout_forgotten_sessions():
    """Automatically check out members who forgot to check out based on org settings"""
    try:
        db = get_db()
        cursor = db.cursor()
        ...
        cursor.execute('''
            SELECT c.id, c.checkin_time, c.organization_id, s.auto_checkout_hours 
            FROM checkins c
            JOIN checkin_settings s ON c.organization_id = s.organization_id
        ...
        for checkin in active_checkins:
            checkin_id, checkin_time_str, org_id, auto_checkout_hours = checkin
            checkin_time = datetime.strptime(checkin_time_str, '%Y-%m-%d %H:%M:%S')
            
            # If they have exceeded the auto-checkout hours, check them out automatically
        ...
        print(f"Error running auto-checkout job: {e}")

def run_scheduler():
    """Run the scheduler in a separate thread"""
    schedule.every().day.at("09:00").do(check_expiring_memberships)
    schedule.every().hour.do(auto_checkout_forgotten_sessions)
...
def send_password_reset_email(email, token, username):
    ...
    try:
        # Prefer the environment variable if explicitly set
    ...
    try:
        ...
        cursor.execute('''
            SELECT m.membership_id, m.name, m.email, m.phone, m.membership_type,
                   ...
                   m.created_at, o.name as org_name, m.birthdate
            FROM members m
            JOIN organizations o ON m.organization_id = o.id
        ...
        # Smart Alerts for the Receptionist
        receptionist_alerts = []
        if days_until_expiry is not None and 0 <= days_until_expiry <= 5:
            receptionist_alerts.append(f"⚠️ Membership expires in {days_until_expiry} days.")
        if prepaid_balance['current_balance'] < 10.0: # Arbitrary low balance threshold
            receptionist_alerts.append("💳 Low prepaid balance.")
        
        # Birthday check
        if member.get('birthdate') and member['birthdate'][5:] == datetime.now().strftime('%m-%d'):
            receptionist_alerts.append("🎉 Today is their birthday!")
...
def api_member_login():
    ...
    Returns ALL memberships tied to that email so the member app
    can show an org picker when the user belongs to multiple orgs.
    """
    try:
        data = request.get_json()
        if not data:
        ...
        # Find every membership row that shares this email
        cursor.execute('''
            SELECT
                m.membership_id,
                m.name,
            ...
            WHERE LOWER(m.email) = ?
            ORDER BY m.status DESC, o.name ASC
        ''', (email,))

        rows = cursor.fetchall()

        if not rows:
            return jsonify({'success': False, 'error': 'No account found with that email'}), 401

        # Verify password — use the first row that has a password_hash set.
        # All memberships for the same person should share the same password.
        password_verified = False
        for row in rows:
            if row['password_hash']:
                if check_password_hash(row['password_hash'], password):
                    password_verified = True
                break   # Only check the first hashed password found

        if not password_verified:
            return jsonify({'success': False, 'error': 'Incorrect password'}), 401

        # Build the memberships list for the frontend
        memberships = []
        for r in rows:
            memberships.append({
                ...
                'organization_name': r['organization_name'] or 'Organization',
                'photo_url': url_for(
                    'member_photo',
                    membership_id=r['membership_id'],
                    _external=True
        ...
        return jsonify({
            ...
            # Convenience: also return count so frontend can branch easily
...
def api_member_register():
    ...
    try:
        ...
        # Check if email or phone already exists in the same organization
        cursor.execute('''
            SELECT id FROM members 
            WHERE email = ? AND organization_id = ?
        ''', (email, organization_id))
        if cursor.fetchone():
            return jsonify({'success': False, 'error': 'You are already registered with this organization'}), 409
        ...
        if cursor.fetchone():
            return jsonify({'success': False, 'error': 'Phone number already registered with this organization'}), 409
...
def api_set_password():
    """
    Set or update password for a member.
    Accepts either email (updates all rows with that email) or
    membership_id (updates just that one row).
    """
    try:
        data          = request.get_json()
        email         = data.get('email', '').strip().lower()
        ...
        if not email and not membership_id:
            return jsonify({'success': False, 'error': 'Email or membership_id required'}), 400

        db     = get_db()
        hashed = generate_password_hash(password)

        if email:
            # Update ALL memberships sharing this email (same person, multiple orgs)
            db.execute(
            ...
            db.execute(
                'UPDATE members SET password_hash = ? WHERE membership_id = ?',
                (hashed, membership_id)
            )
